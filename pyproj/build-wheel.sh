...
for whl in "$ABS_OUTPUT_DIR"/pyproj-[0-9]*.whl; do
  if [[ -f "$whl" ]] && [[ "$whl" != *".post"* ]] && [[ "$whl" != *"-1td-"* ]]; then
...
RAW_WHEELS=$(ls -1 "$ABS_OUTPUT_DIR"/pyproj-[0-9]*.whl 2>/dev/null | grep -v "\.post" | grep -v "\-1td\-" || echo "")
...
#      the primary proj.db in the wheel and adds a build tag "1td" to the wheel filename.
...
# We need to exclude any already-patched wheels (which have .post or build tag -1td- in the name)
all_wheels = list(Path("/dist").glob("pyproj-*.whl"))
raw_wheels = [w for w in all_wheels if ".post" not in w.name and "-1td-" not in w.name]
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    # Note: Only post_patch goes in the version; time_dependent becomes a build tag
    filename_suffix = ""
    if post_patch:
    ...
    wheel.unlink()
    if filename_suffix or build_tag:
        parts = wheel.name.split("-")
        # Format: pyproj-{version}(-{build_tag})?-{python}-{abi}-{platform}.whl
        # Original parts: ["pyproj", "3.7.2", "cp312", "cp312", "linux_x86_64.whl"]
        # With .post1: ["pyproj", "3.7.2.post1", "cp312", "cp312", "linux_x86_64.whl"]
        # With build tag: ["pyproj", "3.7.2.post1", "1td", "cp312", "cp312", "linux_x86_64.whl"]
        
        if filename_suffix:
            parts[1] = f"{parts[1]}{filename_suffix}"
        if build_tag:
            parts.insert(2, build_tag)  # Insert build tag after version
...
if [[ -n "$TIME_DEPENDENT" ]]; then
  # For TD variant, expect build tag "1td" in filename
  CREATED_WHEELS=$(ls -1 "$ABS_OUTPUT_DIR"/pyproj*.post${POST_PATCH}-1td-* 2>/dev/null | wc -l)
  if [[ "$CREATED_WHEELS" -eq 0 ]]; then
    echo "ERROR: No time-dependent wheel with build tag '1td' was created!"
    echo "Wheels in $ABS_OUTPUT_DIR:"
    ls -lh "$ABS_OUTPUT_DIR"/pyproj*.whl 2>/dev/null || echo "No wheels found"
    exit 1
  fi
elif [[ -n "$POST_PATCH" ]]; then
  # For standard variant, expect .post in version and no build tag
  CREATED_WHEELS=$(ls -1 "$ABS_OUTPUT_DIR"/pyproj*.post${POST_PATCH}-cp* 2>/dev/null | wc -l)
  if [[ "$CREATED_WHEELS" -eq 0 ]]; then
    echo "ERROR: No standard wheel with post version was created!"
    echo "Wheels in $ABS_OUTPUT_DIR:"
    ls -lh "$ABS_OUTPUT_DIR"/pyproj*.whl 2>/dev/null || echo "No wheels found"
    exit 1
  fi
